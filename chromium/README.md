...
## Reactive Controller — cách chia sẻ logic trong Lit

Ở Polymer, logic có state và lifecycle thường được chia sẻ bằng **mixin**:

```javascript
class UserPage extends LoadableMixin(PolymerElement) {}
```

Lit vẫn hỗ trợ class mixin, nhưng **Reactive Controller** phù hợp hơn với phần lớn logic dùng chung. Controller là object độc lập do component sở hữu, có state và tham gia update lifecycle của host.

```text
Mixin:      component is-a mixin     → logic đi vào prototype chain
Controller: component has-a object   → logic và API nằm trong object riêng
```
...
| API | Chức năng |
|-----|-----------|
| `host.addController(this)` | Đăng ký controller với host |
| `host.requestUpdate()` | Yêu cầu host render lại khi state của controller thay đổi |
| `hostConnected()` | Khởi tạo listener, observer hoặc subscription |
| `hostDisconnected()` | Dọn listener, observer hoặc subscription |
| `hostUpdate()` | Chạy ngay trước `update()` và `render()` của host |
| `hostUpdated()` | Chạy sau khi DOM được cập nhật |

Field của controller không tự reactive như property của `LitElement`; controller phải gọi `requestUpdate()` sau khi thay đổi state có ảnh hưởng đến giao diện.

Một component có thể sở hữu nhiều controller, kể cả nhiều instance cùng loại, mà không kéo dài inheritance chain hoặc gây trùng tên method.

### Lựa chọn controller hoặc mixin

| Reactive Controller | Mixin |
|---------------------|-------|
| Feature nội bộ của component | Public API phải nằm trực tiếp trên component |
| State và lifecycle được cô lập | Cần can thiệp chi tiết vào class lifecycle |
| Cần nhiều instance cùng loại | Component phải kế thừa behavior |

### Ví dụ: theo dõi trạng thái online
...
class OnlineStatusController implements ReactiveController {
  private readonly host_: ReactiveControllerHost;
  ...
  hostConnected() {
    window.addEventListener('online', this.onStatusChange_);
    window.addEventListener('offline', this.onStatusChange_);
  }

  hostDisconnected() {
    window.removeEventListener('online', this.onStatusChange_);
    window.removeEventListener('offline', this.onStatusChange_);
  }

  private readonly onStatusChange_ = () => {
...
class NetworkIndicatorElement extends LitElement {
  private readonly network_ = new OnlineStatusController(this);

  override render() {
    return html`
      <span role="status">
        ${this.network_.online ? '🟢 Online' : '🔴 Offline'}
      </span>
    `;
  }
}

customElements.define('network-indicator', NetworkIndicatorElement);
```
